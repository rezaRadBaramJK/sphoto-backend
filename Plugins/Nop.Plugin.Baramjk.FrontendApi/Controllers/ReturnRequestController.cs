...
{
    public class ReturnRequestController : BaseNopWebApiFrontendController
    {
        #region Ctor
        ...
        public virtual async Task<IActionResult> CustomerReturnRequests()
        {
            ...
            return ApiResponseFactory.Success(model.ToDto<CustomerReturnRequestsModelDto>());
        }
        ...
        public virtual async Task<IActionResult> ReturnRequest(int orderId)
        {
            ...
            if (order == null || order.Deleted || (await _workContext.GetCurrentCustomerAsync()).Id != order.CustomerId)
                return ApiResponseFactory.BadRequest("This order does not belong to the current customer");

            if (!await _orderProcessingService.IsReturnRequestAllowedAsync(order))
                return ApiResponseFactory.BadRequest("Return request is not allowed.");

            var model = new SubmitReturnRequestModel();
            model = await _returnRequestModelFactory.PrepareSubmitReturnRequestModelAsync(model, order);

            return ApiResponseFactory.Success(model.ToDto<SubmitReturnRequestModelDto>());
        }

        [HttpPost("{orderId}")]
        [ProducesResponseType(typeof(SubmitReturnRequestModelDto), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(string), StatusCodes.Status400BadRequest)]
        public virtual async Task<IActionResult> ReturnRequestSubmit(
            [FromBody] BaseModelDtoRequest<SubmitReturnRequestModelDto> request, int orderId)
        {
            var order = await _orderService.GetOrderByIdAsync(orderId);
            if (order == null || order.Deleted || (await _workContext.GetCurrentCustomerAsync()).Id != order.CustomerId)
                return ApiResponseFactory.BadRequest("This order does not belong to the current customer");
            ...
            //returnable products
            var orderItems = await _orderService.GetOrderItemsAsync(order.Id, false);
            foreach (var orderItem in orderItems)
            {
                ...
                var rrr = await _returnRequestService.GetReturnRequestReasonByIdAsync(returnRequestModel
                    .ReturnRequestReasonId);
                var rra = await _returnRequestService.GetReturnRequestActionByIdAsync(returnRequestModel
                ...
                var rr = new ReturnRequest
                {
                    ...
                    ReasonForReturn = rrr != null
                        ? await _localizationService.GetLocalizedAsync(rrr, x => x.Name)
                        : "not available",
                    RequestedAction = rra != null
                        ? await _localizationService.GetLocalizedAsync(rra, x => x.Name)
                        : "not available",
                ...
                //notify store owner
                await _workflowMessageService.SendNewReturnRequestStoreOwnerNotificationAsync(rr, orderItem, order,
            ...
            returnRequestModel =
                await _returnRequestModelFactory.PrepareSubmitReturnRequestModelAsync(returnRequestModel, order);
            if (count > 0)
                returnRequestModel.Result = await _localizationService.GetResourceAsync("ReturnRequests.Submitted");
            else
                returnRequestModel.Result =
                    await _localizationService.GetResourceAsync("ReturnRequests.NoItemsSubmitted");

            return ApiResponseFactory.Success(returnRequestModel.ToDto<SubmitReturnRequestModelDto>());
        }
            ...
            [FromQuery] [Required] string contentType)
        {
            if (!_orderSettings.ReturnRequestsEnabled || !_orderSettings.ReturnRequestsAllowFiles)
                return ApiResponseFactory.Success(new UploadFileResponse
                    { Success = false, DownloadGuid = Guid.Empty });
