...
namespace Nop.Web.Factories
{
    ...
    /// </summary>
    public partial class ProductModelFactory2 : IProductModelFactory
    {
        #region Fields
        ...
        public ProductModelFactory2(CaptchaSettings captchaSettings,
            ...
            VendorSettings vendorSettings, IShoppingCartService shoppingCartService,
        ...
        /// </returns>
        protected virtual async Task<ProductOverviewModel.ProductPriceModel> PrepareProductOverviewPriceModelAsync(
        ...
        /// </returns>
        protected virtual async Task<ProductDetailsModel.ProductBreadcrumbModel> PrepareProductBreadcrumbModelAsync(
            Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        /// </returns>
        protected virtual async Task<ProductDetailsModel.ProductPriceModel> PrepareProductPriceModelAsync(
            Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            if (await _permissionService.AuthorizeAsync(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    {
                        var (oldPriceBase, _) = await _taxService.GetProductPriceAsync(product, product.OldPrice);
                        var (finalPriceWithoutDiscountBase, _) = await _taxService.GetProductPriceAsync(product,
                            (await _priceCalculationService.GetFinalPriceAsync(product,
                                await _workContext.GetCurrentCustomerAsync(), includeDiscounts: false)).finalPrice);
                        ...
                        model.BasePricePAngV =
                            await _priceFormatter.FormatBasePriceAsync(product, finalPriceWithDiscountBase);
                        //currency code
        ...
        /// </returns>
        protected virtual async Task<ProductDetailsModel.AddToCartModel> PrepareProductAddToCartModelAsync(
            Product product, ShoppingCartItem updatecartitem)
        {
            ...
            if (product.OrderMinimumQuantity > 1)
            {
                model.MinimumQuantityNotification = string.Format(
                    await _localizationService.GetResourceAsync("Products.MinimumQuantityNotification"),
            ...
            if (product.AvailableForPreOrder)
            {
                    ...
                    _catalogSettings.DisplayDatePreOrderAvailability)
                {
                    model.PreOrderAvailabilityStartDateTimeUserTime =
                        (await _dateTimeHelper.ConvertToUserTimeAsync(model.PreOrderAvailabilityStartDateTimeUtc.Value))
                        .ToString("D");
                }
            }
            ...
            model.CustomerEnteredPrice = updatecartitem != null
                ? updatecartitem.CustomerEnteredPrice
                : minimumCustomerEnteredPrice;
            model.CustomerEnteredPriceRange = string.Format(
                await _localizationService.GetResourceAsync("Products.EnterProductPrice.Range"),
        ...
        /// </returns>
        protected virtual async Task<IList<ProductDetailsModel.TierPriceModel>> PrepareProductTierPriceModelsAsync(
            Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));

            var model = await (await _productService.GetTierPricesAsync(product,
                    await _workContext.GetCurrentCustomerAsync(), (await _storeContext.GetCurrentStoreAsync()).Id))
                .SelectAwait(async tierPrice =>
                {
                    var priceBase = (await _taxService.GetProductPriceAsync(product,
                        (await _priceCalculationService.GetFinalPriceAsync(product,
                            await _workContext.GetCurrentCustomerAsync(), decimal.Zero,
                            _catalogSettings.DisplayTierPricesWithDiscounts,
                            tierPrice.Quantity)).Item1)).price;

                    var price = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(priceBase,
                        await _workContext.GetWorkingCurrencyAsync());
        ...
        /// </returns>
        protected virtual async Task<IList<ManufacturerBriefInfoModel>> PrepareProductManufacturerModelsAsync(
            Product product)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        /// </returns>
        protected virtual async Task<(PictureModel pictureModel, IList<PictureModel> allPictureModels)>
            PrepareProductDetailsPictureModelAsync(Product product, bool isAssociatedProduct)
        {
            ...
            var cachedPictures = await _staticCacheManager.GetAsync(productPicturesCacheKey, async () =>
            {
                ...
                (imageUrl, defaultPicture) =
                    await _pictureService.GetPictureUrlAsync(defaultPicture, defaultPictureSize, !isAssociatedProduct);
                ...
                for (var i = 0; i < pictures.Count(); i++)
                {
                    ...
                    (imageUrl, picture) =
                        await _pictureService.GetPictureUrlAsync(picture, defaultPictureSize, !isAssociatedProduct);
                    (fullSizeImageUrl, picture) = await _pictureService.GetPictureUrlAsync(picture);
        ...
        protected virtual async Task<IList<ProductTag>> GetProductTagsAsync(Product product)
        {
        ...
        protected virtual async Task<List<CategoryItemModel>> GetProductCategoriesAsync(Product product)
        {
            var categoryItemModels = new List<CategoryItemModel>();
            var productCategories = await _categoryService.GetProductCategoriesByProductIdAsync(product.Id);
            foreach (var productCategory in productCategories)
            {
                var category = await _categoryService.GetCategoryByIdAsync(productCategory.CategoryId);
                var categoryItemModel = new CategoryItemModel
                {
                    Id = category.Id,
                    Name = category.Name
                };
        ...
        /// </returns>
        public virtual async Task<IEnumerable<ProductOverviewModel>> PrepareProductOverviewModelsAsync(
            IEnumerable<Product> products,
            bool preparePriceModel = true, bool preparePictureModel = true,
            int? productThumbPictureSize = null, bool prepareSpecificationAttributes = false,
            bool forceRedirectionAfterAddingToCart = false, bool prepareTag = true, bool prepareCategories = true)
        {
            if (products == null)
            ...
            var vendors = _vendorRepository.Table.Where(item => vendorIds.Contains(item.Id))
                .Select(item => new ProductOverviewModel.VendorItemDto {Id = item.Id, Name = item.Name})
            ...
            foreach (var product in products)
            {
                ...
                {
                    model.DefaultPictureModel =
                        await PrepareProductOverviewPictureModelAsync(product, productThumbPictureSize);
                }
                ...
                model.HasAttribute = mappings.Any(item => item.ProductId == product.Id);
                model.HasRequiredAttribute = mappings.Any(item => item.ProductId == product.Id && item.IsRequired);
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            (model.DefaultPictureModel, allPictureModels) =
                await PrepareProductDetailsPictureModelAsync(product, isAssociatedProduct);
            ...
            if (product.IsGiftCard)
            {
                ...
                if (updatecartitem == null)
                {
                    model.GiftCard.SenderName =
                        await _customerService.GetCustomerFullNameAsync(await _workContext.GetCurrentCustomerAsync());
            ...
            if (_shippingSettings.EstimateShippingProductPageEnabled && !model.IsFreeShipping)
            {
                ...
                var estimateShippingModel =
                    await _shoppingCartModelFactory.PrepareEstimateShippingModelAsync(new[] {wrappedProduct});
                ...
                if (!isAssociatedProduct)
                {
                    ...
                    foreach (var associatedProduct in associatedProducts)
                        model.AssociatedProducts.Add(
                            await PrepareProductDetailsModelAsync(associatedProduct, null, true));
                }
            }
        ...
        /// </returns>
        public virtual async Task<ProductReviewsModel> PrepareProductReviewsModelAsync(ProductReviewsModel model,
            Product product)
        {
            if (model == null)
            ...
            var productReviews = (await _productService.GetAllProductReviewsAsync(
                approved: true,
                productId: product.Id,
                storeId: _catalogSettings.ShowProductReviewsPerStore
                    ? (await _storeContext.GetCurrentStoreAsync()).Id
                    : 0)).AsEnumerable();
            ...
            foreach (var reviewType in await _reviewTypeService.GetAllReviewTypesAsync())
            {
                model.ReviewTypeList.Add(new ReviewTypeModel
                {
                    ...
                    Description =
                        await _localizationService.GetLocalizedAsync(reviewType, entity => entity.Description),
            ...
            foreach (var pr in productReviews)
            {
                ...
                var productReviewModel = new ProductReviewModel
                {
                    ...
                    AllowViewingProfiles = _customerSettings.AllowViewingProfiles && customer != null &&
                    ...
                    WrittenOnStr = (await _dateTimeHelper.ConvertToUserTimeAsync(pr.CreatedOnUtc, DateTimeKind.Utc))
                        .ToString("g"),
                };
                ...
                if (_customerSettings.AllowCustomersToUploadAvatars)
                {
                    productReviewModel.CustomerAvatarUrl = await _pictureService.GetPictureUrlAsync(
                        ...
                        _mediaSettings.AvatarPictureSize, _customerSettings.DefaultAvatarEnabled,
                        defaultPictureType: PictureType.Avatar);
                }

                foreach (var q in await _reviewTypeService.GetProductReviewReviewTypeMappingsByProductReviewIdAsync(
                    pr.Id))
                {
                    ...
                    productReviewModel.AdditionalProductReviewList.Add(new ProductReviewReviewTypeMappingModel
                    {
                        ...
                        VisibleToAllCustomers = reviewType.VisibleToAllCustomers ||
                                                (await _workContext.GetCurrentCustomerAsync()).Id == pr.CustomerId,
                    });
                }
            ...
            model.AddProductReview.CanCurrentCustomerLeaveReview =
            ...
            model.AddProductReview.DisplayCaptcha =
                _captchaSettings.Enabled && _captchaSettings.ShowOnProductReviewPage;
        ...
        public virtual async Task<CustomerProductReviewsModel> PrepareCustomerProductReviewsModelAsync(int? page)
        {
            ...
            foreach (var review in list)
            {
                ...
                var productReviewModel = new CustomerProductReviewModel
                {
                    ...
                    WrittenOnStr = (await _dateTimeHelper.ConvertToUserTimeAsync(review.CreatedOnUtc, DateTimeKind.Utc))
                        .ToString("g")
                };
                ...
                if (_catalogSettings.ProductReviewsMustBeApproved)
                {
                    productReviewModel.ApprovalStatus = review.IsApproved
                        ? await _localizationService.GetResourceAsync(
                            "Account.CustomerProductReviews.ApprovalStatus.Approved")
                        : await _localizationService.GetResourceAsync(
                            "Account.CustomerProductReviews.ApprovalStatus.Pending");
                }
        ...
        /// </returns>
        public virtual async Task<ProductEmailAFriendModel> PrepareProductEmailAFriendModelAsync(
            ProductEmailAFriendModel model, Product product, bool excludeProperties)
        {
            if (model == null)
                throw new ArgumentNullException(nameof(model));
