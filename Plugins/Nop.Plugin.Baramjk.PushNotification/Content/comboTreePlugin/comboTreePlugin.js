...
;(function ($, window, document, undefined) {
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        ...
        if (isThereSubs)
            itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse || collapse ?
                '<span class="mdi mdi-chevron-right-circle-outline fas fa-angle-right"></span>' :
                '<span class="mdi mdi-chevron-down-circle-outline fas fa-angle-down"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';
        var inputCheckbox = sourceItem.selected === true ? '<input type="checkbox"  />' : '<input type="checkbox" />';
        if (this.options.isMultiple)
            itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">'
                + (!selectableLastNode && isSelectable ? inputCheckbox : '') + sourceItem.title + '</span>';
        else
    ...
    ComboTree.prototype.toggleSelectionTree = function (item, direction) {
        ...
        if (direction === undefined) {
            if ($(subMenu).is(':visible'))
                $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-right-circle-outline fas fa-angle-right"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
            else
                $(item).children('span.comboTreeParentPlus').html('<span class="mdi mdi-chevron-down-circle-outline  fas fa-angle-down"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
