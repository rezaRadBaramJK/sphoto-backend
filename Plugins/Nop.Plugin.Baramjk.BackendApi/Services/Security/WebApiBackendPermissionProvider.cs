...
namespace Nop.Plugin.Baramjk.BackendApi.Services.Security
{
    /// <summary>
    ///     Web API Backend permission provider
    /// </summary>
    public class WebApiBackendPermissionProvider : IPermissionProvider
    {
        ...
        public virtual HashSet<(string systemRoleName, PermissionRecord[] permissions)> GetDefaultPermissions()
        {
            return new HashSet<(string systemRoleName, PermissionRecord[] permissions)>
                { (NopCustomerDefaults.AdministratorsRoleName, new[] { AccessWebApiBackend }) };
        }
    }
}