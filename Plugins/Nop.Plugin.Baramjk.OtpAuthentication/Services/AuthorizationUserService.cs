    ...
    public class AuthorizationUserService : IAuthorizationUserService
    {
        ...
        public virtual async Task<AuthenticateResponse> AuthenticateAsync(AuthenticateCustomerRequest request)
        {
            ...
            if (string.IsNullOrEmpty(request.PhoneNumber) == false)
                request.PhoneNumber = PhoneNumberHelper.RemoveFirstBelongAtTheBeginningOfThePhoneNumber(request.PhoneNumber);
            ...
            if (result.LoginResults == CustomerLoginResults.Successful)
            {
                ...
                var resp = await GenerateAuthenticateResponse(result.Customer);
                return resp;
            }
        ...
        public virtual async Task<Customer> GetCustomerAsync(AuthenticateCustomerRequest request)
        {
            ...
            {
                var customerQuery =
                    from attribute in _repositoryGenericAttribute.Table
                    where attribute.KeyGroup == nameof(Customer) &&
                          attribute.Key == NopCustomerDefaults.PhoneAttribute &&
                          attribute.Value == request.PhoneNumber

                    join c in _customerRepository.Table on attribute.EntityId equals c.Id
                    where c.Deleted == false && c.Active
                        select c;
            ...
            AuthenticateCustomerRequest request, bool checkPasswordsMatch = true)
        {
            if (customer == null)
            {
                return new CustomerLoginResultsModel(null, CustomerLoginResults.CustomerNotExist);
            }
        ...
        {
            if (_otpAuthenticationSettings.RefreshTokenLifetimeSeconds > 0)
            {
                return DateTimeOffset.Now.AddSeconds(_otpAuthenticationSettings.RefreshTokenLifetimeSeconds)
                    .ToUnixTimeSeconds();
            }

            return DateTimeOffset.Now.AddDays(30).ToUnixTimeSeconds();
        }

        public virtual string GetRefreshToken(Customer customer)
        {
            var expiresInSeconds = GetRefreshTokenExpire();
